<?php 
    ...
    if(isset($_POST['submit'])){
        $mail = isset($_SESSION['cusMail']) ? $_SESSION['cusMail'] : "";
        $card = isset($_SESSION['cusIdCard']) ? $_SESSION['cusIdCard'] : "";
        $pass1 = isset($_POST['pass1'])?$_POST['pass1'] :"99"; 
        $pass2 = isset($_POST['pass1'])?$_POST['pass1'] :"11";
        if($pass1 == $pass2){
            $lastPassword = md5($pass2);
        }
        if(isset($lastPassword)){
            $sql = "UPDATE customer SET cusPassword = '$lastPassword' WHERE Email = '$mail' AND cusIdCard = '$card'";
